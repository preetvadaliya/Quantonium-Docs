...
Array.from(document.querySelectorAll("[block-type = helper]")).forEach(element => {
  let helperName = element.getAttribute("helper-name");
  let workspace = Blockly.inject(element.id, { scrollbars: false, readOnly: true });
  let block = workspace.newBlock('helper');
  block.initHelper(helperName);
  renderBlock(block, element);
});

// Render event's param blocks.
Array.from(document.querySelectorAll("[block-type = getter]")).forEach(element => {
  let vName = element.getAttribute("variable-name");
  let workspace = Blockly.inject(element.id, { scrollbars: false, readOnly: true });
  let block = workspace.newBlock('getter');
  block.initGetter(vName);
  renderBlock(block, element);
});

// Render event's param blocks.
Array.from(document.querySelectorAll("[block-type = setter]")).forEach(element => {
  let vName = element.getAttribute("variable-name");
  let workspace = Blockly.inject(element.id, { scrollbars: false, readOnly: true });
  let block = workspace.newBlock('setter');
  block.initSetter(vName);
  renderBlock(block, element);
});
